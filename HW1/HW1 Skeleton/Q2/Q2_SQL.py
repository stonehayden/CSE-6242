...
class HW2_sql():
    ...
    # GTusername [0 points]
    def GTusername(self):
        gt_username = "shayden33"
        return gt_username
    
    # Part a.i Create Tables [2 points]
    def part_ai_1(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        # creating table named movies with columns id (integer), title (text), score (real)
    ...
    def part_ai_2(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        # creating table named movie_cast with columns movie_id (integer), cast_id (integer), cast_name (text),
        # birthday (text), popularity (real)
        part_ai_2_sql = """
    ...
    def part_aii_1(self,connection,path):
        ...
        with open(path, 'r', encoding="utf8") as f:
            ...
            for row in csv_reader:
                ...
                cursor.execute("INSERT INTO movies VALUES (?,?,?)", (row[0], row[1], row[2]))
        ######################################################################
    ...
    def part_aii_2(self,connection, path):
        ...
        with open(path, 'r', encoding="utf8") as f:
            ...
            for row in csv_reader:
                ...
                cursor.execute("INSERT INTO movie_cast VALUES (?,?,?,?,?)", (row[0], row[1], row[2], row[3], row[4]))
    ...
    def part_aiii(self,connection):
        ...
        ############### CREATE IMPORT CODE BELOW ############################
        # importing data to vertically partitioned table cast_bio from movie_cast by removing duplicates
        # and keeping only the first row of each duplicate
    ...
    def part_c(self,connection):
        ...
        FROM movies WHERE score > 50 AND title LIKE '%war%' COLLATE NOCASE;
        """
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        # # adding column total_cast to movies table
        # part_e_sql = """
        # ALTER TABLE movies ADD COLUMN total_cast integer;
        # """
        # connection.execute(part_e_sql)
        #
        # # inserting number of cast
        # part_e_sql = """
        # UPDATE movies
        # SET total_cast = (SELECT COUNT(*) FROM movie_cast WHERE movie_id = movies.id);
        # """
        # connection.execute(part_e_sql)
        #
        # # finding the Highest Scoring Movies With the Least Amount of Cast
        # # 5 highest-scoring movies that have the fewest cast members
        # # format is (movie_id, movie_title, score, cast_count).
        # query = """
        # SELECT movies.id, movies.title, movies.score, movies.total_cast
        # FROM movies
        # ORDER BY movies.score DESC, movies.title ASC, movies.total_cast ASC
        # LIMIT 5;
        # """
        # results = connection.execute(query).fetchall()
        #
        # # removing column total_cast from movies table
        # part_e_sql = """
        # ALTER TABLE movies DROP COLUMN total_cast;
        # """

        part_e_sql = """
        SELECT m.title, m.score, COUNT(*)
        FROM movies m
        INNER JOIN movie_cast mc ON m.id = mc.movie_id
        GROUP BY m.title
        ORDER BY m.score DESC, COUNT(*) ASC
        LIMIT 5;
        """
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = """
        SELECT cast_id, cast_name, printf("%.2f", AVG(score)) AS score
        FROM (
            SELECT cast_id, cast_name, movie_id, score
            FROM movie_cast mc, movies m
    ...
    def part_g(self, connection):
        ...
        SELECT a.cast_id AS cast_member_id1,b.cast_id AS cast_member_id2,COUNT(a.movie_id) AS movie_count, 
        printf('%.2f',avg(c.score)) AS average_movie_score 
        FROM movie_cast a,movie_cast b,movies c 
        WHERE a.movie_id =b.movie_id And a.cast_id <> b.cast_id and a.movie_id=c.id 
        GROUP BY a.cast_id,b.cast_id 
        HAVING COUNT(a.movie_id)>2 AND avg(c.score)>=40"""
        ######################################################################
        return self.execute_query(connection, part_g_sql)

    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = """SELECT cast_id,cast_name, avg(score) AS collaboration_score 
    ...
    def part_h(self,connection,path):
        ############### EDIT SQL STATEMENT ###################################
        # Import movie overview data from the movie_overview.csv into a new FTS table called
        # movie_overview with the schema: id INTEGER, overview TEXT.

        part_h_sql = """
        CREATE VIRTUAL TABLE IF NOT EXISTS movie_overview USING fts4(id, overview);
        """
        ######################################################################
        ...
        with open(path, 'r', encoding="utf8") as f:
            ...
            for row in csvreader:
                id = row[0]
                overview = str(row[1]).lower()
                insert_sql = """
                INSERT INTO movie_overview VALUES (?,?);
                """
                connection.execute(insert_sql, (id, overview))
        ######################################################################
